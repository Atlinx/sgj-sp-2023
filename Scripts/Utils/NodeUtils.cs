...
public static class NodeUtils
{
    ...
    public static bool IsInEditorSceneTab(Node node)
    {
        if (Engine.IsEditorHint())
        {
            ...
            {
                node = node.GetParent();
                if (node is Viewport && node.GetParent() is SubViewportContainer && node.GetParent()?.GetParent() is Control)
                    return true;
            }
        }
        return false;
    ...
    public static Node Reparent(this Node node, Node newParent, bool keepGlobalTransform = true)
    {
        ...
        Transform3D globalTransform = default;
    ...
    {
        return node.GetImmediateChild<T>(false);
    }

    /// <summary>
    /// Wrapper for GetImmediateChildren. Components of a node are the immediate children of that node.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="node"></param>
    /// <returns></returns>
    public static T[] GetComponents<T>(this Node node)
    {
        return node.GetImmediateChildren<T>();
    }

    /// <summary>
    /// Wrapper for TryGetImmediateChild. Tries to get a component for a node. Components of a node are the immediate children of that node.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="node"></param>
    ...
    {
        return node.TryGetImmediateChild<T>(out component, false);
    }

    /// <summary>
    /// Wrapper for HasImmediateChild. Components of a node are the immediate children of that node.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="node"></param>
    /// <returns></returns>
    ...
    {
        return node.HasImmediateChild<T>(false);
    }

    /// <summary>
    /// Wrapper for AddImmediateChild. Components of a node are the immediate children of that node.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="node"></param>
    /// <returns></returns>
    ...
    {
        return node.AddImmediateChild<T>();
    }

    /// <summary>
    /// Wrapper for AddChild. Components of a node are the immediate children of that node.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="node"></param>
    /// <param name="component"></param>
    public static void AddComponent<T>(this Node node, T component) where T : Node
    {
